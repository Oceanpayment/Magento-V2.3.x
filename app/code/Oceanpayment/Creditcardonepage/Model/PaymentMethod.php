...
{
    const CODE = 'oceanpaymentcreditcardonepage';
    ...
    protected $_formBlockType = 'Oceanpayment\Creditcardonepage\Block\Form';
    protected $_infoBlockType = 'Oceanpayment\Creditcardonepage\Block\Info';
    ...
    {
        return $this->urlBuilder->getUrl('oceanpaymentcreditcardonepage/payment/redirect', ['_secure' => true]);
    }
    ...
    public function assignData(\Magento\Framework\DataObject $data)
    {
        ...
        // 获取前端传递的 custom_field 值
        $additionalData = $data->getData('additional_data');

        // 将值保存到 payment 对象中
        $this->getInfoInstance()->setAdditionalInformation('custom_field', $additionalData);
    ...
    public function getCheckoutParameter()
    {
        ...
        //card_data
        $card_data  = $additionalData['custom_field']['card_data']??'';
        ...
        if($this->getConfigData('secure/secure_mode') == 1){
            ...
            $validate_arr['pay_mode'] = $this->getConfigData('pay_mode');
        }
        ...
        //返回地址
        $backUrl           = $this->urlBuilder->getUrl('oceanpaymentcreditcardonepage/payment/back', ['_secure' => true,'_nosid' => true]);
        //服务器响应地址
        $noticeUrl         = $this->urlBuilder->getUrl('oceanpaymentcreditcardonepage/payment/notice', ['_secure' => true,'_nosid' => true]);
        //备注
        $order_notes       = $orderIncrementId;
        //账单人名        
        $billing_firstName = $billing->getFirstname()?substr(urlencode($this->OceanHtmlSpecialChars($billing->getFirstname())),0,50):'N/A';        
        //账单人姓
        $billing_lastName  = $billing->getLastname()?substr(urlencode($this->OceanHtmlSpecialChars($billing->getLastname())),0,50):'N/A';               
        //账单人email
        ...
        $parameter = array('account'=>$account,
            ...
            'cart_info'=>'Magento|V1.0.0|'.$isMobile,
            'cart_api'=>'V1.0',
            'pages'=>$pages,
        );
        ...
        $url_pay = $validate_arr['pay_mode'];
        $result_data = $this->curl_send($url_pay,$parameter);
        //记录请求返回数据
        $this->resultLog('Credit Card onepage request result',$result_data);
        ...
        if (strtolower($local_signValue) == strtolower($back_signValue)) {
            ...
            $history = ' (payment_id:'.$payment_id.' | order_number:'.$order_number.' | '.$order_currency.':'.$order_amount.' | payment_details:'.$payment_details.')';
            $payment = $order->getPayment();
            $payment->setCcTransId($payment_id);
            $payment->setLastTransId($payment_id)->setIsTransactionClosed(0);
            
            if($payment_status == 1 || $payment_status == 2){ //支付成功
                //支付成功
                $order->setState($this->getConfigData('success_order_status'));
                $order->setStatus($this->getConfigData('success_order_status'));
                $order->addStatusToHistory($this->getConfigData('success_order_status'), __(self::BrowserReturn.'Payment Success!'.$history));

                $order->save();

                //保存sales->transactions支付记录
                $payment->setLastTransId($payment_id)->setTransactionId($payment_id)->setIsTransactionClosed(true)
                ->setShouldCloseParentTransaction(true);
                $transaction = $payment->addTransaction(\Magento\Sales\Model\Order\Payment\Transaction::TYPE_CAPTURE, null, true, "");
                $transaction->setIsClosed(true);
                $transaction->save();
                // $this->messageManager->addSuccessMessage(__('Payment Success! '.$payment_details));

                $array_data = array(
                    'result' => 'success',
                    'msg' => 'Payment Success! '.$payment_details,
                    'url'   => 'checkout/onepage/success'
                );
                return json_encode($array_data);
            
            }elseif($payment_status == 0){  //失败
                //支付失败
                $order->setState($this->getConfigData('failure_order_status'));
                $order->setStatus($this->getConfigData('failure_order_status'));
                $order->addStatusToHistory($this->getConfigData('failure_order_status'), __(self::BrowserReturn.'Payment Failed!'.$history));
                $order->save();
                ...
                // $this->messageManager->addErrorMessage(__('Payment Failed! '.$payment_details));
            
                $array_data = array(
                    'result' => 'fail',
                    'msg' => 'Payment Failed! '.$payment_details,
                ...
                //交易待处理
                $order->setState($this->getConfigData('pre_auth_order_status'));
                $order->setStatus($this->getConfigData('pre_auth_order_status'));
                $order->addStatusToHistory($this->getConfigData('pre_auth_order_status'), __(self::BrowserReturn.'(Pre-auth)Payment Pending!'.$history));
                $order->save();
            ...
            }elseif($payment_status == 1000){  //预授权
                //10000:Payment is declined 高风险订单
                $order->setState($this->getConfigData('high_risk_order_status'));
                $order->setStatus($this->getConfigData('high_risk_order_status'));
                $order->addStatusToHistory($this->getConfigData('high_risk_order_status'), __(self::BrowserReturn.'(High Risk)Payment Failed!'.$history));
                $order->save();
     ...
     * post log
     */
    public function resultLog($logType, $data){
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $directoryList = $objectManager->get(DirectoryList::class);
        $varPath = $directoryList->getPath(DirectoryList::VAR_DIR);
